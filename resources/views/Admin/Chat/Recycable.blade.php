...
<div class="content-frame " id="myChat">
      ...
      <div class="content-frame-left">
        ...
        <div class="list-group list-group-contacts border-bottom push-down-10" dir="rtl"  >
            <a href="#" class="list-group-item" v-for="user in users" v-on:click="choose_user(user)" dir="rtl">
                ...
                <span class="contacts-title" v-if="user.user_name" > @{{user.user_name}} </span>
    ...
    <!-- START CONTENT FRAME BODY -->
      <div class="content-frame-body content-frame-body-right ">
        ...
        <div class="messages messages-img" id="chat_container">

            <div  v-for="chat in the_chat" v-bind:class="['item', {'in' : checkifAdmin(chat)  } , 'item-visible' ] " >
                <div class="image">
                    <img src="{{asset('atlant/assets/images/users/user.jpg')}}" v-if="chat.type == 'user'">
                    <img src="{{asset('atlant/assets/images/users/user2.jpg')}}" v-else>
                </div>
                <div class="text mydirection">
                    <div class="heading ">
                        <a href="#" v-if="chat.type == 'admin'"> مشرف </a>
                        <a v-else >
                              <span v-if="current_user_model.user_name"> @{{current_user_model.user_name}} </span>
                              <span v-if="current_user_model.model"> @{{current_user_model.model}} </span>
                        </a>
...
@section('script')
    ...
    <script>
        var magazine_image_path = '{{asset('images/categories')}}';
        var search_path = '{{asset('Category/search')}}';
        var list_path = '{{asset('Category')}}';
        var api_get_chat_users = '{{url('Chat/get_chat_users')}}';
