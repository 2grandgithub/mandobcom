...
@component('components.panel_default_with_blank')
    ...
    @slot('page_title') @lang('page.City')  @endslot
    @slot('panel_title') @lang('page.City') @endslot
    ...
    @slot('body')
       ...
       {{--    <button class="btn btn-primary btn-rounded" v-on:click="showCreateModel()">
          ...
          <div class="col-md-6 mydirection">
            {!! Form::model($User = new \App\City,[ 'id'=>'search_form' ,'v-on:submit.prevent'=>'getResults()']) !!}
...
{{--
    ...
    @slot('script')
        <script>
            var delete_api = '{{url('City/delete')}}';
            var get_list = '{{url('City/list')}}';
            var showORhide_api = '{{url('City/showORhide')}}';
            var create_api = '{{url('City/create')}}';
            var update_api = '{{url('City/update')}}';
        </script>
