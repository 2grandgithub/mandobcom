  ...
  methods:{
       getResults(page = 1){
         ...
         this.show_spinner = true;
          ...
          if(this.first_getResults_call_finshed)
          {
          ...
          {
               if(get_company_id_search_val)
                 company_ids.push(get_company_id_search_val);
          }
          ...
          $.post(base_url+'/item_list'+'?page=' + page ,search_data,(Response)=>{
              root.mainList = Response.items;
              root.companies = Response.Companies;
              root.show_spinner = false;

              if(!root.first_getResults_call_finshed) // fix for default company value comes from anther page
              {
                 if(get_company_id_search_val)
                 {
                    let l = `.inp_company[value=${get_company_id_search_val}]` ;
                    setTimeout(()=>{   $(l).attr('checked','checked');   },500);
                 }
                root.first_getResults_call_finshed = true;
              }
          });
       },
       switch_view(view)
