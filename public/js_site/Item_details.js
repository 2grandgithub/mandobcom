...
let root = new Vue({
  ...
  methods:{
       ...
       get_comments_and_related_items()
       {
            ...
            $.post(`${base_url}/Action/get_comments_and_related_items`,sendData,(responce)=>{
       ...
       like_add_or_remove()
       {
           ...
           {
                 $.ajax({
                       url: `${base_url}/Action/like_add_or_remove/item/${this.item.item_id}`,
                       headers: {
                       ...
                       dataType: 'json',
                       success: function(responce){
                           if(responce.status=='unValidToken'){                       console.log('f');
                               new Noty({ text: ' يجب عليك تسجيل الدخول', layout: 'topRight', type: 'info' , timeout:'2000' }).show();
                           }
                           else if (responce.status=='success')
                           {
