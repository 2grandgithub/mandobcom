...
class ChatController extends Controller
{
    ...
    {
        return view('Admin.Chat.index',compact('type'));
    }
   ...
   //-------------------------Start get users--------------------------------
    ...
    public function get_chat_users_type_Recycable() // Recycable
    {
        $users = Chat::select('name','recycables.id as user_id','recycables.model',
                              'chat.type','message_type','message' )
                     ->leftJoin('recycables','recycables.id','chat.user_id')->where('chat.type','Recycable')->whereNull('admin_id')
                     ->distinct('chat.user_id')->groupBy('chat.user_id')->latest('chat.id')->get();
    ...
    public function get_chat_users_type_ProducerFamily() // ProducerFamily
    {
        $users = Chat::select('name','producer_family.id as user_id','producer_family.model',
                              'chat.type','message_type','message' )
                     ->leftJoin('producer_family','producer_family.id','chat.user_id')->where('chat.type','ProducerFamily')->whereNull('admin_id')
                     ->distinct('chat.user_id')->groupBy('chat.user_id')->latest('chat.id')->get();
  ...
  //-------------------------End get users--------------------------------
    ...
    public function store(Request $request)
    {
      ...
      // dd($request->all());
        ...
        if ($firebase_tokens)
        {
            $responce = \App\PushNotification::chat($firebase_tokens,$request->type,'admin',$request->message ); //return $responce;
        }
